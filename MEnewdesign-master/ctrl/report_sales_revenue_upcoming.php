	...
	if(isset($_POST['formSubmit']))
{
	...
	//Display list of Successful Transactions
	 $cardTrCond=" and e.enddatetime >now() and e.status = 1 ".$Catid." ".$citySql." ".$SalesId;
	$TransactionRES=$cCtrl->getTransactionsByEventAndOrgWiseAndRevenueUpcoming($ExtraCharge,$cardTrCond,$orgcitySql);
        ...
        if(isset($_POST['exportReports'])){
            ...
            while($value=$TransactionRES->fetch_assoc()){
			  ...
			  if($value['mecommission']!=""){
			   ...
			   $finalArr[$value['UserID']]['TotRevenue'][$value['code']]+=$mere+$orre+$value['ExAmount'];
            }
...
include 'templates/report_sales_revenue_upcoming.tpl.php';
?>