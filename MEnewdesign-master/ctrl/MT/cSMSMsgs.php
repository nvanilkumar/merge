...
class cSMSMsgs
{
	private $TableNm = "messagetemplate";

	public $Id = 0;
	public $Msg = '';
	public $MsgType = '';
//	public enum('MsgType', array('smRegn', 'smEvent', 'smForgotPass'));

	//----------------------------------------------------------------------------------------------------

	public function __construct($lId, $sMsg, $sMsgType)
	{
		$this->Id 		=  $lId;
		$this->Msg 		=  $sMsg;
	...
	public function Save()
	{
		...
		try
		{
			...
			if ($EditedRecord)
			{
				$Success = $stmt->bind_param("ssi", $this->Msg, $this->MsgType, $this->Id);    // Bind the parameters
			}
			else
			{
				$Success = $stmt->bind_param("ss", $this->Msg, $this->MsgType);
		  // Bind the parameters
			}
	...
	public function Load()
	{
		...
		try
		{
			...
			$myQuery="SELECT Id, Msg, MsgType FROM ".$this->TableNm." WHERE Id = ?";
			$stmt = $Conn->stmt_init();    // Create a statement object
			...
			$Success = $stmt->bind_result($this->Id, $this->Msg, $this->MsgType);	// Bind the result parameters
