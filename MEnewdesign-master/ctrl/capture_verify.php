...
if ($_POST['submit_form'] == "Submit") {
    ...
    $ebs_comm_records = array();
    foreach ($ebs_captured_data as $ebs_record) {
        if ($ebs_record[1] == "Captured") {
            $ebs_capture_array[] = $ebs_record[0];
        } else if ($ebs_record[1] == "Refunded") {
            $ebs_refund_array[] = $ebs_record[0];
        }elseif($ebs_record[1]=="TDR"){
            $amt=  abs($ebs_record[2]);
            if($amt<0){
                $amt*=-1;
            }
            $ebs_comm_records[$ebs_record[0]]=$amt;
        }
    }
    ...
    for ($i = 0; $i < $ebs_loop_count; $i++) {
        ...
        if (strlen($capture_event_singup_ids) > 1) {
            $status = 'Verified';
            change_event_signup_status($Global, $capture_event_singup_ids, $status, $trasaction_date,$ebs_comm_records);
        }
...
include 'templates/capture_verify_tpl.php';
?>