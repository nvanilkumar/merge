...
//	}
	...
	{
	//$sqlTransComments="select * from CancelTransComments where EventSIgnupId=".$TransId;
            $sqlTransComments="select `comment`,`cts` from comment where eventsignupid=".$TransId;
	$indId = $Global->SelectQuery($sqlTransComments); 
	}
?>
...
<div >
    ...
    <td><div><?=$indId[$i]['comment'];?></div><div align="right" style="color:#007788; font-family:Arial, Helvetica, sans-serif; font-size:13px;">Posted on: <?=date('d F Y',strtotime($indId[$i]['cts']));?></div></td>
  </tr>
	<?	}
		?>
</table>
<?php
if($_REQUEST['pagename']=="OnlyCancelTrans")
{
$action="OnlyCancelTrans.php?EventId=".$_REQUEST[EventId]."&Status=".$_REQUEST[Status]."&settleDt=".$_REQUEST[settleDt]."&endDt=".$_REQUEST[endDt];
}elseif($_REQUEST['pagename']=="CheckCODTrans")
{
    $action="CheckCODTrans.php?EventId=".$_REQUEST['EventId']."&Status=".$_REQUEST['Status']."&txtSDt=".$_REQUEST['txtSDt']."&txtEDt=".$_REQUEST['txtEDt'];
}else{
$action="CancelTrans.php?EventId=".$_REQUEST[EventId]."&Status=".$_REQUEST[Status];
}
?>
	    <form name="add_comment" action="<?=$action;?>"  id="add_comment" method="post" style="margin:0px; padding:0px;">
        ...
        <table width="800" cellpadding="0" cellspacing="0">
         <tr>
                ...
                <td width="677" height="30" align="left"><textarea name="comment" rows="10" cols="30" id="comment" ><?=($txtName)?$txtName : date('d F Y  h:i').":"  ?></textarea> 
