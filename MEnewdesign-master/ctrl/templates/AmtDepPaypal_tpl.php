...
<body style="background-image: url(images/background.gif); background-repeat:repeat-x; margin-top: 0px; margin-left: 0px; margin-right:0px; padding:0px">
  ...
  <tr>
    ...
    <form action="AmtDepPaypal.php" method="post" name="frmEofMonth">
    <table width="70%" align="center" class="tblcont">
 <tr><td align="center">Paypal Settlement Report</td></tr>
	<tr>
...
<table width='100%' border='1' cellpadding='0' cellspacing='0'  >
	...
	{ ?>
		...
		$CountQueryt = "SELECT s.id AS Id,s.signupdate AS SignupDt,s.quantity AS Qty, (s.totalamount/s.quantity) AS Fees, s.eventid AS EventId, c.code AS currencyCode ,s.conversionrate AS conversionRate FROM eventsignup as s INNER JOIN currency c ON c.id=s.fromcurrencyid where 1  and s.depositdate!='0000-00-00 00:00:00'  AND  s.paymenttransactionid != 'A1' AND s.paymentgatewayid='4' $dates and s.eventid='".$TransactionRES[$i]['eventid']."'"; 
            $CountQueryRESt=$Global->SelectQuery($CountQueryt); 
            for($j = 0; $j < count($CountQueryRESt); $j++)
            {
               $currencyCode=$CountQueryRESt[$j]['currencyCode'];
             if($CountQueryRESt[$j]['conversionRate']!=1 && $CountQueryRESt[$j]['currencyCode']!="INR"){
                         $currencyCode="INR";
                     }
               $Totalreg=$Totalreg+1;
               $Totalqty=$Totalqty+$CountQueryRESt[$j][Qty];
               $TotaltAmountcard[$currencyCode]+=($CountQueryRESt[$j][Qty]*$CountQueryRESt[$j][Fees]*$CountQueryRESt[$j][conversionRate]);
               $totTransAmt[$currencyCode]+=($CountQueryRESt[$j][Qty]*$CountQueryRESt[$j][Fees]*$CountQueryRESt[$j][conversionRate]);
            }
		
	/* $CountQuery = "SELECT  Id, SignupDt, Qty, Fees,c.code 'currencyCode' FROM eventsignup as s INNER JOIN currency c ON c.id=s.fromcurrencyid  where 1  and s.depositdate!='0000-00-00 00:00:00'   $dates AND (Fees != 0 AND paymenttransactionid != 'A1'  and eChecked != 'Refunded') and EventId='".$TransactionRES[$i]['eventid']."'"; 
	 $CountQueryRES=$Global->SelectQuery($CountQuery); 
	 for($j = 0; $j < count($CountQueryRES); $j++)
	{ 
	$TotalAmountcard[$CountQueryRES[$j]['currencyCode']]+=($CountQueryRES[$j][Qty]*$CountQueryRES[$j][Fees]);
	
	}*/	
            //Refunded entries we check the settlement date
            $dates2=" and date(s.SettlementDate) = '".$yesterdaySDate."' ";
            $CountQueryr = "SELECT s.id AS Id,s.signupdate AS SignupDt,s.quantity AS Qty, (s.totalamount/s.quantity) AS Fees, s.eventid AS EventId, c.code AS currencyCode ,s.conversionrate AS conversionRate FROM eventsignup as s INNER JOIN currency c ON c.id=s.fromcurrencyid where 1 and s.depositdate!='0000-00-00 00:00:00' AND s.paymentgatewayid='4' $dates2  AND (s.totalamount != 0 AND s.paymenttransactionid != 'A1'   and s.paymentstatus = 'Refunded') and s.eventid='".$TransactionRES[$i]['eventid']."'"; 
	    $CountQueryRESr=$Global->SelectQuery($CountQueryr); 
            for($j = 0; $j < count($CountQueryRESr); $j++)
            { 
                 $currencyCode=$CountQueryRESr[$j]['currencyCode'];
                  if($CountQueryRESr[$j]['conversionRate']!=1 && $CountQueryRESr[$j]['currencyCode']!="INR"){
                         $currencyCode="INR";
                     }
                 $TotalrAmountcard[$currencyCode]+=($CountQueryRESr[$j][Qty]*$CountQueryRESr[$j][Fees]*$CountQueryRESr[$j][conversionRate]);
                 $totRefndAmt[$currencyCode]+=($CountQueryRESr[$j][Qty]*$CountQueryRESr[$j][Fees]*$CountQueryRESr[$j][conversionRate]);
           }
	
          
          
           
           $CountQueryVer = "SELECT s.id AS Id,s.signupdate AS SignupDt,s.quantity AS Qty, (s.totalamount/s.quantity) AS Fees, s.eventid AS EventId, c.code AS currencyCode ,s.conversionrate AS conversionRate FROM eventsignup as s INNER JOIN currency c ON c.id=s.fromcurrencyid where 1 and s.depositdate!='0000-00-00 00:00:00'  $dates  AND (s.totalamount != 0 AND s.paymenttransactionid != 'A1' AND s.paymentgatewayid='4' and s.paymentstatus = 'Verified') and s.eventid='".$TransactionRES[$i]['eventid']."'"; 
	   $CountQueryRESVer=$Global->SelectQuery($CountQueryVer); 
           for($j = 0; $j < count($CountQueryRESVer); $j++)
           {
                ...
                if($CountQueryRESVer[$j]['conversionRate']!=1 && $CountQueryRESVer[$j]['currencyCode']!="INR"){
                         $currencyCode="INR";
                     }
                $TotalAmountverified[$currencyCode]+=($CountQueryRESVer[$j][Qty]*$CountQueryRESVer[$j][Fees]*$CountQueryRESVer[$j][conversionRate]);
                $totVrfdAmt[$currencyCode]+=($CountQueryRESVer[$j][Qty]*$CountQueryRESVer[$j][Fees]*$CountQueryRESVer[$j][conversionRate]);
           }
...
<!-------------------------------ADD CONTENT PAGE ENDS HERE--------------------------------------------------------------->
	<?php
        	...
        	if($_REQUEST['txtSDt']!=""){
                        $TAmountBC=$TAmount=$TDeposit=NULL;
        
                        if(strlen(trim($EventId))>0)
                        {
                            $eventcondition=" and EventId=$EventId "; //adding condition to query related to Event
                        }

                        $DepositQuery = "SELECT s.id AS Id,s.signupdate AS SignupDt,s.quantity AS Qty, (s.totalamount/s.quantity) AS Fees, s.eventid AS EventId, c.code AS currencyCode ,s.conversionrate AS conversionRate FROM eventsignup AS s INNER JOIN currency c ON c.id=s.fromcurrencyid where 1 and s.depositdate!='0000-00-00 00:00:00'  AND  s.paymenttransactionid != 'A1' AND s.paymentgatewayid='4' AND s.paymentstatus!='Refunded' ". $dates . $eventcondition . $SearchQuery  ." order by s.eventid DESC"; 
                        ...
                        <p>&nbsp;</p>
                                <table width='90%' border='1' cellpadding='0' cellspacing='0' >
                                    ...
                                    <td class='tblinner' valign='middle' width='6%' align='center'>Amount</td>
                                     <td class='tblinner' valign='middle' width='6%' align='center'>Converted Amount</td>
...
//        if($DepositRES[$i]['conversionRate']!=1 && $DepositRES[$i]['currencyCode']!="INR"){
//                         $currencyCode="INR";
//                     }
                                ?>
                                        <tr>
                        ...
                        ?>
                                                ...
                                                <?php 
                                                $currencyCodeNew=$currencyCode;
                                                 $am1=($am*$DepositRES[$i]['conversionRate']);
                                                 if($DepositRES[$i]['conversionRate']!=1 && $DepositRES[$i]['currencyCode']!="INR"){                                                                             $currencyCodeNew="INR";
                                                 }?>
                                                <td class='tblinner' valign='middle' width='6%' align='right'><font color='#000000'><?php echo $currencyCodeNew." ".$am1?></font></td> 
                                   <?php if($DepositRES[$i][SignupDt] > '2012-03-31 11:59:59')
                                                {  ?>
                                                 <td class='tblinner' valign='middle' width='6%' align='right'><font color='#000000'><?php echo $currencyCode." ".round($am1-($am1*0.0241),2);?></font></td>    	
                                                <?php }else{ ?>
                                                 <td class='tblinner' valign='middle' width='6%' align='right'><font color='#000000'><?php echo $currencyCode." ".round($am1-($am1*0.0238),2);?></font></td>    	
                                        <?php	} ?>
                                  ...
                                  <?php
                                  //$am1=($am*$DepositRES[$i]['conversionRate']);
                                $TAmount[$currencyCodeNew]+=($am1);
                                $TAmountBC[$currencyCode]+=($am);
                                         if($DepositRES[$i][SignupDt] > '2012-03-31 11:59:59')
                                                { 
                                        $TDeposit[$currencyCodeNew]+=round($am1-($am1*0.0241),2);
                                        }else{
                                        $TDeposit[$currencyCodeNew]+=round($am1-($am1*0.0238),2);
                                        }
                                }
                                  $TAmountStr=  totalStrWithCurrencies($TAmount);
                                  $TDepositStr=  totalStrWithCurrencies($TDeposit);
                                  $TAmountBCStr=totalStrWithCurrencies($TAmountBC);
                                ?>
                                <tr><td colspan="5" style="line-height:30px;"><strong>Total   Amount:</strong></td><td  align='right'><font color='#000000'> <?php echo $TAmountBCStr;?></font></td><td  align='right'><font color='#000000'> <?php echo $TAmountStr;?></font></td><td  align='right'><font color='#000000'> <?php echo $TDepositStr;?></font></td></tr>
