...
jQuery(document).ready(function($) {
	...
	$("#keyWord").autocomplete({
        source: function(request, response) {
            $.ajax({
                ...
                type: "GET",
                dataType: "json",
                data: { term: request.term, microsite:'holi',subcategory:164 },
	   ...
	   $('#searchButton').on('click',function (e) {
		   window.location = currentSite+'search?keyWord='+encodeURI($("#SearchText").val());
	   });
	...
	$('#loadmore').on('click',function (e) {
		...
		$.ajax({
            url: '/api/event/list?countryId=14&categoryId=1&subcategoryId=164&day=6&page=1&limit=1&eventMode=0&eventType=nonwebinar&'+cityAndState,
            type: "GET",
            dataType: "json",
            success: function(data) {
...
function renderEvents(data) {
    ...
    {
		var eventThumb = (event.thumbImage.length > 0)?event.thumbImage:'http://static.meraevents.com/microsites/holi/assets/img/holi-2016-default.jpg';
		
        var event = data.response.events[i];
        html += '<li><div class="wrapper-article">';
        //html += '<a href="'+event.eventUrl+'" class="thumbnail">';
        html += '<img src="'+eventThumb+'" width="" height="" alt="'+event.title+'" title="'+event.title+'" onerror="http://static.meraevents.com/microsites/holi/assets/img/holi-2016-default.jpg" />';
        html += '<a class="hover" target="_blank" href="'+event.eventUrl+'">';
        html += '<span class="category">BOOK NOW</span><span class="line-h"></span>'; 
        html += '<h3>'+event.eventUrl+'</h3>';
        html += '<p>'+event.startDate+' | '+event.cityName+'</p> ';
		
        html += '</a></div>';
        html += '<div class="eventname">'+event.title+'<span class="city">'+event.cityName+'</span></div></li>';
