...
if($cityname=="Other")
{
	$sqlBangCityCk="select Id from `Cities` where `city` like '%bangalore%' or city like '%bengaluru%' ";
	$dataBangCityCk=$Globali->SelectQuery($sqlBangCityCk);
	
	$bangCityIdStrCk=NULL;
	if(count($dataBangCityCk)>0)
	{
		foreach($dataBangCityCk as $bangKeyCk=>$bangCityIdCk)
		{
			$bangCityIdStrCk.=$bangCityIdCk['Id'].",";
		}
		$bangCityIdStrCk=",".substr($bangCityIdStrCk,0,-1);
	}
}
...
if(isset($_COOKIE['CityName']) && $cityname=="" ) { 
   if($_COOKIE['CityName']=="Other")
   {
	...
	$conCity=" AND CityId not in(47,14,37,39,77,448 $bangCityIdStrCk) And StateId not in (53,11) ";   
   }else{
