...
$sql = "select CONCAT(e.Title, ', in ', c.City) as label, e.Id 
	from events as e 
	Inner join Cities as c on e.CityId=c.Id 
	where  e.Title LIKE '%$queryString%' and (StartDt > ADDDATE(now(), INTERVAL -10 DAY) and EndDt > now()) and CategoryId=8 and e.Published=1 and e.Private=0  limit 10";
